...
if (FEATURE_RANKING_PLOT_DEPICTION == TRUE) {
        ...
        folderName <- "../results/"
        mkdirResultsCommand <- paste0("mkdir -p .", folderName)
        system(mkdirResultsCommand)
        cat("applied command: ", mkdirResultsCommand, "\n", sep="")
        x_upper_lim <- -1
          
         barPlotOfRanking(aggregateRankings, aggregateRankings$MeanDecreaseAccuracy, aggregateRankings$features, aggregateRankings$firstColPos, exe_num, "features", "MeanDecreaseAccuracy", x_upper_lim, folderName)
         
         barPlotOfRanking(aggregateRankings, aggregateRankings$MeanDecreaseGini, aggregateRankings$features, aggregateRankings$secondColPos, exe_num, "features", "MeanDecreaseGini", x_upper_lim, folderName)
