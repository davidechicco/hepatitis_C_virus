...
ranker <- function(data, train_indices, method="relief")
{
    ...
    xx <- x[,which(names(x) != "cirrhosis", arr.ind = TRUE)]
    
    if(method=="relief"){
        tmp<-relief(cirrhosis~.,x)
        ranked_list <- rownames(tmp[order(tmp$attr_importance,decreasing = TRUE),,drop=FALSE])
    }
    
    if(method=="MMPC"){
        library("MXM")
        tmp <- MMPC(target=as.vector(x$cirrhosis),dataset = xx)
        ranked_list <- names(xx)[order(tmp@stats, decreasing = TRUE)]
    }
    
    if(method=="RF"){
        tmp <- random.forest.importance(cirrhosis~.,x,importance.type = 1)
        ranked_list <- rownames(tmp[order(tmp$attr_importance,decreasing = TRUE),,drop=FALSE])
    }
    
    if(method=="oneR"){
        tmp <- oneR(cirrhosis~.,x)
...
#         print(tmp)
    ...
    if(method=="xgboost"){
        train <- list(data=xx,label=x$cirrhosis)
...
classif <- function(method, the_train, the_test, feats)
{
  ...
  if(method=="rf"){
    dd<-randomForest(as.factor(cirrhosis)~ ., data=the_train,ntree=1000)
    cf_tr <- dd$confusion[1:2,1:2]
    cf_ts <- confusionMatrix(as.factor(predict(dd,the_test)),as.factor(the_test$cirrhosis))$table
    
    predicted_values <- predict(dd,the_test)
    actual_values <- the_test$cirrhosis
    }
  
  if(method=="svm"){
    aa <- ksvm(cirrhosis~.,data=the_train,type='C-svc',C=100000,kernel="rbfdot")
    cf_tr <- confusionMatrix(as.factor(aa@fitted),as.factor(aa@ymatrix))$table
    cf_ts <- confusionMatrix(as.factor(predict(aa,the_test)),as.factor(the_test$cirrhosis))$table
  ...
  if(method=="xgb"){
    ee <- xgboost(data = as.matrix(the_train[,feats]), label = the_train$cirrhosis,nrounds = 25, silent=0,objective = "binary:logistic", verbose=0)
    cf_tr <- confusionMatrix(as.factor(as.numeric(predict(ee,as.matrix(the_train[,feats]))>CF_TAU)),as.factor(the_train$cirrhosis))$table
    cf_ts <- confusionMatrix(as.factor(as.numeric(predict(ee,as.matrix(the_test[,feats]))>CF_TAU)),as.factor(the_test$cirrhosis))$table
    
#     print("confusionMatrix(as.factor(as.numeric(predict(ee,as.matrix(the_test[,feats]))>CF_TAU)),as.factor(the_test$cirrhosis))")
#     print(confusionMatrix(as.factor(as.numeric(predict(ee,as.matrix(the_test[,feats]))>CF_TAU)),as.factor(the_test$cirrhosis)))
#     
...
names(all_data)
all_features <- names(all_data)[!names(all_data) %in% "cirrhosis"]
...
attach(all_data)
idx_class0 <- which(cirrhosis==0, arr.ind = TRUE)
idx_class1 <- which(cirrhosis==1, arr.ind = TRUE)
...
for(i in 1:n_sets){
    ...
    this_train <- all_data[splits[[i]][["TR"]], c(top_feats, "cirrhosis"), drop=FALSE]
    this_test <- all_data[splits[[i]][["TS"]], c(top_feats, "cirrhosis"), drop=FALSE]
