...
# package loading
list.of.packages <- c("easypackages", "FSelector", "caret", "MXM", "e1071", "rpart", "xgboost", "randomForest", "kernlab", "mltools", "boot", "PRROC", "mice")
...
datasetFile <- "../data/hcvdat0_EDITED_fibrosis_cirrhosis.csv"
targetName <- "category_0fibrosis_1cirrhosis"
MISSING_DATA_IMPUTATION <- TRUE

# datasetFile <- "../data/journal.pone.0118297_S1_Dataset_HPV_EDITED_cirrhosis.csv"
...
results_by_method <- function(thisSplits, algorithm) 
{
    ...
    algorithm_PPV_rates <- unlist(lapply(thisSplits, FUN = function(x){x[["top_feats"]][[algorithm]][["TS"]][["PPV"]]}))
    algorithm_NPV_rates <- unlist(lapply(thisSplits, FUN = function(x){x[["top_feats"]][[algorithm]][["TS"]][["NPV"]]}))
   ...
   if(algorithm == "xgb") algorithmCompleteName <- "Gradient boosting (XGBoost)"
    ...
    CI_algorithm_PPV_rates <- confidence_intervals(algorithm_PPV_rates)
    CI_algorithm_NPV_rates <- confidence_intervals(algorithm_NPV_rates)
    
    cat("MCC  accuracy  F1_score  TP_rate  TN_rate PPV NPV PR_AUC   ROC_AUC\n")
    meanIndex <- 2
    cat(dec_three(CI_algorithm_MCCs[meanIndex]), "\t", dec_three(CI_algorithm_accuracies[meanIndex]), "\t", dec_three(CI_algorithm_f1_scores[meanIndex]), "   ", dec_three(CI_algorithm_TP_rates[meanIndex]), "  ", dec_three(CI_algorithm_TN_rates[meanIndex]),  " ", dec_three(CI_algorithm_PPV_rates[meanIndex]), "  ", dec_three(CI_algorithm_NPV_rates[meanIndex]),  " ", dec_three(CI_algorithm_PR_AUCs[meanIndex]),  " ", dec_three(CI_algorithm_ROC_AUCs[meanIndex]), "\n")
...
ranker <- function(data, train_indices, method="relief")
{
    ...
    xx <- x[,which(names(x) != "category_0fibrosis_1cirrhosis", arr.ind = TRUE)]
    
    if(method=="relief"){
        tmp<-relief(category_0fibrosis_1cirrhosis~.,x)
        ranked_list <- rownames(tmp[order(tmp$attr_importance,decreasing = TRUE),,drop=FALSE])
    }
    
    if(method=="MMPC"){
        library("MXM")
        tmp <- MMPC(target=as.vector(x$category_0fibrosis_1cirrhosis),dataset = xx)
        ranked_list <- names(xx)[order(tmp@stats, decreasing = TRUE)]
    }
    
    if(method=="RF"){
        tmp <- random.forest.importance(category_0fibrosis_1cirrhosis~.,x,importance.type = 1)
        ranked_list <- rownames(tmp[order(tmp$attr_importance,decreasing = TRUE),,drop=FALSE])
    }
    
    if(method=="oneR"){
        tmp <- oneR(category_0fibrosis_1cirrhosis~.,x)
...
#         print(tmp)
    ...
    if(method=="rpart"){
        tmp <- rpart(category_0fibrosis_1cirrhosis~.,x)
    ...
    if(method=="lsvm"){
        fit <- svm(category_0fibrosis_1cirrhosis~.,data=x)
    ...
    if(method=="xgboost"){
        train <- list(data=xx,label=x$category_0fibrosis_1cirrhosis)
...
classif <- function(method, the_train, the_test, feats)
{
  ...
  if(method=="rf"){
    dd<-randomForest(as.factor(category_0fibrosis_1cirrhosis)~ ., data=the_train,ntree=1000)
    cf_tr <- dd$confusion[1:2,1:2]
    cf_ts <- confusionMatrix(as.factor(predict(dd,the_test)),as.factor(the_test$category_0fibrosis_1cirrhosis))$table
    
    predicted_values <- predict(dd,the_test)
    actual_values <- the_test$category_0fibrosis_1cirrhosis
    }
  
  if(method=="svm"){
    aa <- ksvm(category_0fibrosis_1cirrhosis~.,data=the_train,type='C-svc',C=100000,kernel="rbfdot")
    cf_tr <- confusionMatrix(as.factor(aa@fitted),as.factor(aa@ymatrix))$table
    cf_ts <- confusionMatrix(as.factor(predict(aa,the_test)),as.factor(the_test$category_0fibrosis_1cirrhosis))$table
  ...
  if(method=="xgb"){
    ee <- xgboost(data = as.matrix(the_train[,feats]), label = the_train$category_0fibrosis_1cirrhosis,nrounds = 25, silent=0,objective = "binary:logistic", verbose=0)
    cf_tr <- confusionMatrix(as.factor(as.numeric(predict(ee,as.matrix(the_train[,feats]))>CF_TAU)),as.factor(the_train$category_0fibrosis_1cirrhosis))$table
    cf_ts <- confusionMatrix(as.factor(as.numeric(predict(ee,as.matrix(the_test[,feats]))>CF_TAU)),as.factor(the_test$category_0fibrosis_1cirrhosis))$table
    
#     print("confusionMatrix(as.factor(as.numeric(predict(ee,as.matrix(the_test[,feats]))>CF_TAU)),as.factor(the_test$category_0fibrosis_1cirrhosis))")
#     print(confusionMatrix(as.factor(as.numeric(predict(ee,as.matrix(the_test[,feats]))>CF_TAU)),as.factor(the_test$category_0fibrosis_1cirrhosis)))
#     
...
#   print(cf_ts)
  ...
  this_PPV_rate <- TP / (TP + FP)
  this_NPV_rate <- TN / (TN + FN)
  ...
  res[["TS"]][["PPV"]] <- this_PPV_rate
  res[["TS"]][["NPV"]] <- this_NPV_rate
...
all_data <- all_data[sample(nrow(all_data)),] # shuffle the rows
...
if(MISSING_DATA_IMPUTATION==TRUE){
    ...
    NUM_DATASETS <- 1
    imputed_data <- mice(all_data, m=NUM_DATASETS, maxit = 50, method = 'pmm', seed = 500)
    all_data <- complete(imputed_data, NUM_DATASETS)
}
...
names(all_data)
all_features <- names(all_data)[!names(all_data) %in% "category_0fibrosis_1cirrhosis"]
...
attach(all_data)
idx_class0 <- which(category_0fibrosis_1cirrhosis==0, arr.ind = TRUE)
idx_class1 <- which(category_0fibrosis_1cirrhosis==1, arr.ind = TRUE)
...
for(i in 1:n_sets){
    ...
    this_train <- all_data[splits[[i]][["TR"]], c(top_feats, "category_0fibrosis_1cirrhosis"), drop=FALSE]
    this_test <- all_data[splits[[i]][["TS"]], c(top_feats, "category_0fibrosis_1cirrhosis"), drop=FALSE]
